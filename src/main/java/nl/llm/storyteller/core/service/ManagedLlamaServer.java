...
public final class ManagedLlamaServer implements AutoCloseable {
  ...
  private ManagedLlamaServer(Process process, int port) {
  ...
  public static ManagedLlamaServer start(nl.llm.storyteller.core.config.LlamaServerConfig config) throws IOException, InterruptedException {
    ...
    Process process = new ProcessBuilder(buildCommand(config, port))
      ...
      .start();
    ManagedLlamaServer server = new ManagedLlamaServer(process, port);
    try {
  ...
  private void waitUntilReady(int startupTimeoutSeconds) throws IOException, InterruptedException {
    ...
    while (Instant.now().isBefore(deadline)) {
      if (!process.isAlive()) {
        throw new IOException("llama-server exited before becoming ready: " + capturedOutput());
      }
      try {
    ...
    throw new IOException("Timed out waiting for llama-server readiness: " + capturedOutput());
  }
